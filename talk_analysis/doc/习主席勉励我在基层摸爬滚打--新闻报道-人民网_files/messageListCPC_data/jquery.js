...
var _rmw_global_ = {
	   		...
	   		success: function(text){
  				...
  				//调用登录后操作
	   			...
	   			sso_page_login_user(guzzLoginUser);
	   			 ...
	   			 sso_page_login_user(null);
	   			}
	        }
	    });
	},
	bindLogin : function(jdom,jDiv,jBg,userName,password){
		 jdom.live("click", function(event){
			     event.preventDefault() ;
			     var isLogin = false; 
	 ...
	 //快速注册
	bindQuickReg : function(jHrefs,jClose,jDiv,jBg){
		jClose.live("click", function(event){
			jDiv.hide();
			jBg.hide();
		 });
		jHrefs.live("click", function(event){
	    var href = $(this).attr("href") ;
	    event.preventDefault() ;
	    $.ajax({
		   	url : href ,
			...
			dataType: "jsonp",
		   	...
		   	success: function(text){
				 if(text=='error'){
					 ...
					 var ajaxObj = eval('(' + text + ')');
					 var userName = ajaxObj.userName;
					 var password = ajaxObj.password;
					 $("#userName").html(userName);
					 $("#password").html(password);
					 jBg.height($(document).height());
					 jBg.show();
					 jDiv.show();
					 var dlk_TOP=window.screen.height/4;
					 var dlk_Left=$(document).width()/2-250;
					 jDiv.animate({top:dlk_TOP},0);
					 jDiv.animate({left:dlk_Left},0);
					 rmw.global.bindLogin($("a[href*='/_login.do']"),jDiv,jBg,userName,password);
				 }
			 }
		});
	    return false;
	...
	bindUserPortraitsByNewBoard: function(jdoms){
		var ssoc = this ;
	...
	translateMillSecondsByNewBoard:function(jdoms){
		var ssoc = this ;
		...
		jdoms.each(function(event){
			...
			var s_ltime = parseInt(s_time);
			var s_date = new Date(s_ltime);
			try{
				//mill-seconds
				...
				if(l_time <= 1000 || l_time == 'NaN' ) {
					...
					if(l_time < 60){
						...
						if(l_time < 60){
							...
							//小时
							l_time = Math.floor(l_time / 60) ;
							var n_date = new Date(s_ltime); ;
							n_date.setHours(23);
							n_date.setMinutes(59);
							n_date.setSeconds(59);
							var n_time=n_date.getTime();
							if(new Date().getTime() < n_time){
								$(this).text("今天 " + format(s_date,"hh:mm")) ;
							}else{
								//天
								l_time = Math.floor(l_time / 24) ;
								if(l_time<365){
									$(this).text(format(s_date,"MM-dd hh:mm")) ;
								}else{
									$(this).text(format(s_date,"yyyy-MM-dd")) ;
								}
							}
						}
					}

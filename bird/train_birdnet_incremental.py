...
def list_dir_filenum():
    image_root_dir = DATA_ROOT_DIR + 'images/'
    subdirs = sorted(os.listdir(image_root_dir))
    min_num = 10000
    for subdir in subdirs:
        num = len(os.listdir(os.path.join(image_root_dir, subdir)))
        if num < min_num:
            min_num = num
    return min_num

def read_images(subdir, image_target_size, subindex=0):
    image_root_dir = DATA_ROOT_DIR + 'images/'
...
def read_class_labels(label_index=0):
    ...
    return np.array(y[label_index*60:(label_index+1)*60])
...
def main():
    ...
    image_root_dir = DATA_ROOT_DIR + 'images/'
    subdirs = sorted(os.listdir(image_root_dir))
    num_output_classes = len(subdirs)
    ...
    sub_index = 0    
    max_index = list_dir_filenum()
    
    for sub_index in range(max_index):
        X = read_images(image_target_size, sub_index)
        y = read_class_labels(sub_index)
